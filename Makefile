...
image_name := smackhoo-voice
registry_url := ssmackey
prod_container_name := voice-prod
...
# Modify Me!
run-local: image
	docker run -t --rm -p6984:6984 ${image_name} ./main.py 6984

# Modify Me!
run-prod: image push
	-ssh dev.ssmackey.com "docker kill ${prod_container_name}"
	ssh dev.ssmackey.com "docker rm -f ${prod_container_name}"
	ssh dev.ssmackey.com "docker pull ${registry_url}/${image_name}"
	ssh dev.ssmackey.com "docker run --name ${prod_container_name} -td --network=host --restart always ${registry_url}/${image_name} ./main.py 6984"
